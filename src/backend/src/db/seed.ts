...
// If scripts/extract_basins.py has been run, load all real basins from the generated file.
// Otherwise fall back to the 12 representative basins for development.
const SEED_FILE = path.resolve(__dirname, '../../basin-seed.json');

const BASINS: {
  hybasId: number; lat: number; lon: number; province: string | null;
  regionId: number | null; elevM: number | null; slopeDeg: number | null;
  twi: number | null; distRiverM: number | null; basinAreaKm2: number | null;
  upAreaKm2: number | null; riverOrder: number | null;
